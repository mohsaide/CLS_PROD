    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET')
    {
        try
        {
           ...
           else
           {
            ...
            $query='select user_friend.id as fu_id  , if (user_auth.value is null , "offline" , "online" ) status , image , concat (first_name , " " , last_name) name from user_friend
            ...
            for ($i = 0; $i < $counter ; $i++) {
                ...
                $friend = array("id"=>  $row["fu_id"] , "status"=>  $row["status"] , "image"=>  $row["image"] , "name"=>  $row["name"]  );
                $data[] = $friend ; 
              }
